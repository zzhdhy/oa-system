...
<head>
    <meta charset="UTF-8">
    <#--网页标题左侧显示-->
    <link href="images/favicon4.ico" rel="icon" type="image/x-icon"/>
    <#--收藏夹显示图标-->
    <link href="images/favicon4.ico" rel="shortcut icon" type="image/x-icon"/>
    ...
    <div class="cont_centrar" style="top:5%;">
        <div class="cont_login">
            <div class="cont_forms cont_forms_active_login" style="height: 480px;width: 400px;border-radius: 15px;">
                <div class="cont_img_back_"><img src="images/login_bg.jpg" alt="背景图片"/></div>
                <form action="logins" method="post" onsubmit="return check();">
                    <div class="cont_form_login" style="display: block;opacity: 1;width: 400px;">
                        ...
                        <h2>登录面板</h2>
                        <input type="text" placeholder="用户名/手机号" autofocus="autofocus" value="${(userName)!''}"
                        ...
                        <div class="login-img" style="display:inline-block">
                            <img class="thisimg" onclick="this.src=this.src.split('?')[0]+'?'+new Date().getTime()"
                                 src="captcha" alt="验证码" style="width: 100px;height: 42px;border-radius: 3px;"
                                 title="点击换一张">
                        </div>
                        <br>
...
<script type="text/javascript">
    ...
    function check() {
        ...
        if (userName == null || userName === "") {
            ...
            $('.alert-danger').css('display', 'block');
            $('.userName').css('border-color', "#DC143C");
            count = 0;
            return false;
        }
