...
public class InformService {
    ...
    private Sort getSort() {
        ...
        orders.addAll(Arrays.asList(new Sort.Order(Direction.DESC, "top"), new Sort.Order(Direction.DESC, "modifyTime")));
    ...
    public Page<NoticesList> pageThis(int page, Long userId, String baseKey, Object type, Object status, Object time) {
        ...
        if (!StringUtils.isEmpty(type)) {
            if ("1".equals(type)) {
                orders.add(new Sort.Order(Direction.DESC, "typeId"));
            } else {
                orders.add(new Sort.Order(Direction.ASC, "typeId"));
            }
        }
        //根据状态排序
        else if (!StringUtils.isEmpty(status)) {
            if ("1".equals(status)) {
                orders.add(new Sort.Order(Direction.DESC, "statusId"));
            } else {
                orders.add(new Sort.Order(Direction.ASC, "statusId"));
            }
        }
        //根据时间排序
        else if (!StringUtils.isEmpty(time)) {
            if ("1".equals(time)) {
                orders.add(new Sort.Order(Direction.DESC, "modifyTime"));
            } else {
                orders.add(new Sort.Order(Direction.ASC, "modifyTime"));
            }
